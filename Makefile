...
check-deadcode:
	uv run vulture src app.py vulture_whitelist.py
...
.PHONY: check format audit check-deadcode ci-check evals install clean run-init run-analyze run-migrate build run-container clean-container generate-docs
