...
class ChefDependencyManager:
    ...
    def __init__(self, cookbook_path: str):
        ...
        self.temp_export_dir: Optional[Path] = None
    ...
    def fetch_dependencies(self) -> None:
        ...
        self.temp_export_dir = Path(tempfile.mkdtemp(prefix="chef-export-"))
        logger.info(f"Created temporary export directory: {self.temp_export_dir}")

        cmd = [
            "chef-cli",
            "export",
            str(self.policy_lock_path),
            str(self.temp_export_dir),
        ]
        ...
        if result.returncode != 0:
            logger.error(f"chef-cli export failed with return code {result.returncode}")
    ...
    def get_dependencies_paths(self, deps: List[CookbookDependency]) -> List[str]:
        ...
        if not self.temp_export_dir:
            logger.warning(
                "No temp export directory set, cannot resolve dependency paths"
            )
            return []

        artifacts_path = self.temp_export_dir / "cookbook_artifacts"
    ...
    def cleanup(self):
        ...
        if not self.temp_export_dir:
            logger.debug("No temporary directory to cleanup")
            return

        if not self.temp_export_dir.exists():
            logger.debug(f"Temporary directory already removed: {self.temp_export_dir}")
            self.temp_export_dir = None
            return

        try:
            logger.info(
                f"Cleaning up temporary export directory: {self.temp_export_dir}"
            )
            shutil.rmtree(self.temp_export_dir)
            logger.info(
                f"Successfully removed temporary directory: {self.temp_export_dir}"
            )
        except Exception as e:
            logger.error(
                f"Failed to cleanup temp directory {self.temp_export_dir}: {e}"
            )
        finally:
            self.temp_export_dir = None
